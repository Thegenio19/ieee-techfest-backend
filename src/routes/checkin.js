...
router.post('/', requireAuth, requireRole('volunteer'), volunteerLimiter, async (req, res) => {
  ...
  // Validation based purely on the checked_in integer flag
  if (ticket.checked_in === 1) {
    throw createError(409, `Already checked in at timestamp ${ticket.checked_in_at}`);
  }

  // Atomic Redis lock to prevent double-scan race conditions at the gate
  const lockKey = `checkin:lock:${registrationId}`;
  const acquired = await redis.set(lockKey, '1', 'NX', 'EX', 30);
  
  if (!acquired) {
    throw createError(409, 'Check-in currently in progress');
  }

  try {
    // Atomic SQLite update: ONLY updates if checked_in is still 0
    const info = db.prepare(`
      UPDATE tickets 
      SET checked_in = 1, checked_in_at = unixepoch(), checked_in_by = ? 
      WHERE id = ? AND checked_in = 0
    `).run(req.user.id, ticket.id);

    if (info.changes === 0) {
      // Lock won, but DB says it was already checked in (another process beat us to the DB)
      throw createError(409, 'Race condition: Already checked in by another volunteer');
    }

    appendEvent('TICKET_CHECKIN', ticket.user_id, { 
      ticketId: ticket.id, 
    ...
    res.status(200).json({
      ...
      checkedInAt: Date.now() // Close enough to unixepoch() for the API response
    });
  } finally {
